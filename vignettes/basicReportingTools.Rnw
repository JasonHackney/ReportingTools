...
We can also publish exicisting images and text directly to sites with \tt hwriter \rm.
...
We can also update our data frame by editing, adding and removing columns with functions.  We then include these functions in our publish call as a list with \tt .modifyDF \rm and \tt .toHTML\rm.
