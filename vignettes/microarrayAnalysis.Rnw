...
Next we use \tt limma \rm to find statistical evidence of differential genes.
...
deReport <- htmlReportRef(shortName = 'de_analysis',
...
If we want to change our images, we can do so with \tt .addColumns \rm (see the basic vignette for more examples of how to use this feature).  In this example, we make lattice plots of the expression of each gene in our table stratified by \tt mol.biol \rm and \tt sex \rm.
...
makeNewImages<-function(df,...){
	...
	for (i in 1:nrow(df)){
		...
		png(filename=paste0("./reports/figuresde_analysis/", imagename[i]))
		print(stripplot(exprs(ALL)[probeId,]~ALL$mol.biol|ALL$sex))
		dev.off()
	}
	df$Image<-hwriteImage(paste0("figuresde_analysis/", imagename), link=paste0("figuresde_analysis/", imagename), table=FALSE, width=100)
	return(df)
}
deReport2<-htmlReportRef(shortName='de_analysis2',
	title = 'Analysis of BCR/ABL translocation differential expression with new plots',
    reportDirectory = "./reports")
publish(fit, deReport2, eSet=ALL, factor=ALL$mol.biol, coef=2, n=100, .addColumns=makeNewImages)
finish(deReport2)
@
...
goReport <- htmlReportRef(shortName = 'go_analysis',
...
PFAMReport <- htmlReportRef(shortName = 'pfam_analysis',
...
geneSetsReport <- htmlReportRef(shortName = "gene_sets",
	title = "Gene Sets", 
	reportDirectory = "./reports")
...
\caption{Resulting page created by \tt publish \rm for \tt geneSets\rm.}
\end{figure}
...
gseaReport <- htmlReportRef(shortName = "gsea_analysis",
	title = "GSEA analysis", 
	reportDirectory = "./reports")
...
We can also add the same statistics via \tt .addColumns \rm.  As demonstrated in the basic vignette, .addColumns allows us to manipulate the output published to our html pages.
...
gseaReport2 <- htmlReportRef(shortName = "gsea_analysis2",
	title = "GSEA analysis", 
	reportDirectory = "./reports")
publish(geneSets, gseaReport2, annotation.db="org.Hs.eg", .addColumns=runGSEA)
finish(gseaReport2)
@
...
indexPage <- htmlReportRef(shortName = "index",
    title = "Analysis of ALL Gene Expression",
    reportDirectory = "./reports",
    baseUrl = "")
publish(Link(list(deReport, goReport)), indexPage)
publish(Link(PFAMReport),indexPage)
publish(Link("GSEA report has a new title", "gsea_analysis.html"), indexPage)
finish(indexPage)
@
\begin{figure}
\centering
\includegraphics[scale = .5]{microarray7.png}
