...
library(hwriter)
htmlRep3 <- HTMLReport(shortName = "my_html_file3", title="Adding an image",
    reportDirectory = "./reports")
    publish(hwrite("This is a link to Bioconductor", link=), htmlRep3) ###ADD LINK
...
We can also publish images and text directly to sites.
<<AddImage, eval=FALSE>>=
...
htmlRep9 <- HTMLReport(shortName = 'my_html_file9', title="Publishing objects that are not data frames",
    reportDirectory = "./reports")
