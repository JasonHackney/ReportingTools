...
If we want to change our images, we can do so with .addColumns (see the basic vignette for more examples of how to use this feature).

#####UPDATE THIS
<<making the DE report, eval=FALSE>>=
deReport <- htmlReportRef(shortName = 'de_analysis',
     title = 'Analysis of AD brain differential expression (images stratified by IL6R vairant)',
     reportDirectory = reportDir)
publish(fit, deReport, eSet=eset, factor=pData(eset)$TS, coef=2, n=50, .addColumns=makeNewImages)
finish(deReport)
@
...
indexPage <- htmlReportRef(shortName = "index",
    ...
    reportDirectory = "./reports",
    baseUrl = "")
publish(Link(c(deReport, goReport)), indexPage)
