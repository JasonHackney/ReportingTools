...
barPlot<-barchart(my.df$RPKM)  ##lattice plots behave slightly differently
...
gscReport <- htmlReportRef(shortName="gsc_example",title="GSEABase Vignette GeneSetCollection", reportDirectory=tempdir(), baseUrl="")
publish(gsc[idx], gscReport, annotation.db="org.Hs.eg")
rr<-finish(gscReport)
